 ...
 * Updated to use external configuration
 */

// Check if configuration is loaded
if (typeof window.STAYFIT_CONFIG === 'undefined') {
    console.error('❌ Configuration not loaded. Please ensure config.js is included before this script.');
    throw new Error('Configuration not loaded');
}

// Validate configuration
if (!window.STAYFIT_CONFIG.validate()) {
    throw new Error('Invalid configuration. Please run the installation script.');
}

// AWS Cognito Configuration
AWS.config.update({ region: window.STAYFIT_CONFIG.cognito.region });

const CognitoAuth = {
    // Configuration from external config
    config: {
        userPoolId: window.STAYFIT_CONFIG.cognito.userPoolId,
        clientId: window.STAYFIT_CONFIG.cognito.clientId,
        clientSecret: window.STAYFIT_CONFIG.cognito.clientSecret,
        identityPoolId: window.STAYFIT_CONFIG.cognito.identityPoolId,
        cognitoDomain: window.STAYFIT_CONFIG.cognito.cognitoDomain,
        redirectUri: window.STAYFIT_CONFIG.app.redirectUri,
        loginUrl: window.STAYFIT_CONFIG.app.loginUrl,
        publicPages: ['login.html'] // Only login page is public
    },
